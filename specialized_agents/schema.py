...
class ContentPlan(BaseModel):
    ...
    why_now: str = Field(description="Why this topic is timely and relevant now")
    key_points: list[str] = Field(description="List of key 5 key statements from the content")
    target_audience: str = Field(description="Who is the target and primary audience for the content")
...
class TopicContent(BaseModel):
    topic: str = Field(description="The selected topic for the content")
    linkedin_post: str = Field(description="The LinkedIn post generated in markdown format")
    twitter_tweets: list[str] = Field(description="A list of 4 distinct tweets for the topic")
    image_generation: ImageGenerationSchema = Field(description="The image generation prompts")
